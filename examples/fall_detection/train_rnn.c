...
// Example: training a plain RNN layer (no gating) to continuously monitor a
// simulated 3-axis accelerometer stream and flag a fall as soon as one
// occurs -- the RNN counterpart to train_gru.c/train_lstm.c in this same
// directory, on the identical task and data, so the three can be compared
// directly. A fall event is a multi-phase pattern (a brief free-fall dip,
// an impact spike, then a long stretch of unusual post-fall stillness)
// spread across a much longer sequence than examples/gesture_recognition's
// fixed windows, with the label VARYING per timestep -- 0 throughout
...
// by resetting the RNN's hidden state (nn_reset_state()): each
...
int main(int argc, char *argv[]) {
  ...
  if (nn == NULL) {
    ...
    // Plain RNN, no gating -- see this file's top comment for why this is
    // expected to be the weakest of the three architectures on this task.
