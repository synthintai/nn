...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    ...
    printf("  input_model:  path to the floating point neural network model (ascii, binary, or inplace)\n");
    printf("  output_model: path where to save the quantized model, in the same format as input_model\n");
    return 1;
  }
  const char *input_model = argv[1];
  const char *output_model = argv[2];

  // Load the original floating-point network. nn_load_model() already
  // auto-detects ascii vs. binary, but the inplace format needs a mutable
  // model to quantize in place (nn_quantize() refuses to run on the
  ...
  // Save the quantized network in the same format the input model was in.
  nn_error_t save_err;
  switch (format) {
